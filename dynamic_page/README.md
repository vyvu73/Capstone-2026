...
A 60-second typing test. React + TypeScript on the front, Express behind it, storing
every result in **both MongoDB Atlas and Postgres**, with passages built from live
quotes off [zenquotes.io](https://zenquotes.io/).
...
```bash
npm install
cp .env.example .env     # fill in MONGODB_URI and/or DATABASE_URL
npm run dev
```
...
Both databases are **optional and independent**. Configure neither and the server
falls back to an in-memory store so the app still runs; configure one and it uses
that one; configure both and every result goes to both. Check which are live:
...
| `node screenshot.mjs http://localhost:3000 [label]` | Screenshot to `temporary screenshots/` |
